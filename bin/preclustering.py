...
def read_mash(filepath : str) -> pd.DataFrame:
    df = pd.read_csv(filepath, sep = '\t', header=None, names = ['query', 'ref', 'dist', 'p-value', 'matches'])
...
def create_graph(df : pd.DataFrame) -> tuple[nx.Graph, dict]:
    G = nx.from_pandas_edgelist(df, source = 'query_clean',
                                 target = 'ref_clean', edge_attr = 'dist')
    pos = nx.circular_layout(G) # To do: add different layout options e.g. spring_layout
    return G, pos


def split_graph(graph : nx.Graph, threshold : float,
                pos : dict, out_dir : Path) -> nx.Graph:
    graph_copy = graph.copy()
    edges_above_threshold = [[n1, n2] for n1, n2, dist in graph_copy.edges(data="dist") if dist > threshold]
    graph_copy.remove_edges_from(edges_above_threshold)
    return graph_copy


def plot_graphs(whole_graph : nx.Graph, splitted_graph : nx.Graph,
                pos : dict, out_dir : Path) -> None:
...
def define_clusters(graph : nx.Graph) -> dict:
    ...
    list_subgraphs = [subgraph for subgraph in sorted(components, key=len, reverse=True)]
    sample_clusters = {}
